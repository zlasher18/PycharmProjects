...
class Dynamics(nn.Module):
    ...
    def forward(state, action):
        ...
        N = len(state)
        state_store = t.zeros((N, 5))
        state_store[:, 1] = t.cos(state[:, 4])
        state_store[:, 3] = -t.sin(state[:, 4])
        delta_state = BOOST_ACCEL * FRAME_TIME * t.mul(state_store, action[:, 0].reshape(-1,1))
...
class Simulation(nn.Module):
    ...
    def error(self, state):
        return t.sum(state**2) # the sum of all the errors squared
...
class Optimize:
    ...
    def visualize(self):
        data = np.array([[self.simulation.state_trajectory[n][k].detach().numpy()
        for n in range(self.simulation.T)]
        for k in range(self.simulation.k)])
        for n in range(self.simulation.k):
            y = data[n, :, 0]
            ydot = data[n, :, 1]
            x = data[n, :, 2]
            xdot = data[n,:, 3]
